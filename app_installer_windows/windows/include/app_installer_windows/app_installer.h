...
class AppInstaller {
    ...
    static string getKnownFolder(GUID folderID){
        ...
        string str = AppInstaller::wstrtostr(wstr);
    ...
    static string getDownloadFolder(){
        return AppInstaller::getKnownFolder(FOLDERID_Downloads);
    }
    static string getDocumentsFolder(){
        return AppInstaller::getKnownFolder(FOLDERID_Documents);
    }
    static HINSTANCE runFileWindows(string filePath){
        string path = filePath.substr(0, filePath.rfind("\\"));
        wchar_t* lpFile = AppInstaller::strtowstr(filePath);
        wchar_t* lpDirectory = AppInstaller::strtowstr(path);
    ...
    static bool getProductAndVersion(string & strProductName, string & strProductVersion){
        ...
        for (int i = 0; i < 4; i++){
            string rsID = rs + AppInstaller::int_to_hex(langCodePages[i][0]) + AppInstaller::int_to_hex(langCodePages[i][1]);
        ...
        if (productNameVersion) {
            ...
            strProductName = AppInstaller::wstrtostr(wstrPN);
            strProductVersion = AppInstaller::wstrtostr(wstrPV);
        }
